"""CLI wrapper around `app.export_set` for batch walking / lateral cuts.
  ...
  python scripts/export_sensecv.py                       # sensecv    walking  (default)
  python scripts/export_sensecv.py supermarket           # supermarket walking
  python scripts/export_sensecv.py sensecv     lateral   # sensecv    lateral
  python scripts/export_sensecv.py supermarket lateral   # supermarket lateral
...
def main():
    ...
    if preset not in app.PRESETS or mode not in ('walking', 'lateral'):
        print(f'usage: python scripts/export_sensecv.py [{ "|".join(app.PRESETS) }] [walking|lateral]')
        return 1
