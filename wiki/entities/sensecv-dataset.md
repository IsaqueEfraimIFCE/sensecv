...
`POST /api/batch-export` (UI's "ExportaÃ§Ã£o em lote" section,
[[viewer-frontend]]). All paths run through `app.py` `export_set()`.

Two **source presets** Ã— two **cut modes** = up to four output folders:

| preset | source set | walking mode â†’ folder | lateral mode â†’ folder |
|---|---|---|---|
...
Recent run counts: SenseCV walking 103/103 ok Â· Supermercado walking 71 ok / 86
no-walking Â· SenseCV lateral 102 ok / 1 below threshold.
...
- **walking** â€” `suggest_crop(idx, 'walking')`, the walking-only segment.
- **lateral** â€” `suggest_lateral_deviation(idx)`, the single longest sustained
  lateral-acceleration burst â‰¥ 0.2 s.
...
- `exports/` is the **labeled** dataset â€” each folder name encodes
...
(supermercado â€” bare folder name, since primary-root clips have no prefix).
...
| `source_root` | Root label â€” the basename of the input root in `EXTRA_CLIP_ROOTS`, or `supermercado` for the primary root |
| `source_subfolder` | The subfolder inside that root |
| `source_path` | Absolute path of the source clip folder |
| `mode` | `walking` or `lateral` â€” which detector produced the cut |
| `start`, `end`, `duration` | Crop window in seconds (relative to source clip start) |
| `status` | `ok` Â· `ok_no_sensor: <err>` Â· `no_segment: <reason>` Â· `ffmpeg_error: <err>` |
...
out_dir, mode)`. They share state â€” `CLIPS`, `CLIP_PATHS`, `WALKING_ONLY`,
`suggest_crop`, `suggest_lateral_deviation`, `save_sensor_data` â€” by living in
the same module rather than re-importing it.

1. Filter clips via `_preset_filter(preset)` â€” `name in WALKING_ONLY` for
...
4. `save_sensor_data(idx, start, end, out_folder)` re-bases the per-frame and
   ...
   export â€” see [[export-pipeline]]).
5. Append a row to `sources.csv`.
...
- `/api/batch-export` is **synchronous** â€” the HTTP request blocks until every
...
- A sensor-save failure is non-fatal (matches `api_crop()` behavior) â€” the
  video still ships and the row records `ok_no_sensor: â€¦`.
