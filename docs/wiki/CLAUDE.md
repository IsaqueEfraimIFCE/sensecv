# SenseCV Wiki — Schema & Maintenance Guide
...
1. **Raw sources** — the actual project, *outside* this `wiki/` directory:
   ...
   short, illustrative excerpts. When code and wiki disagree, the code wins —
   fix the wiki.

2. **The wiki** — this directory. LLM-generated markdown: an overview, entity
   pages (files, data structures, routes), and concept pages (algorithms,
   workflows). The LLM owns this layer entirely.

3. **The schema** — this file. Conventions and workflows. Co-evolve it as the
...
wiki/
  ...
  overview.md          ← project synthesis / entry point
  entities/            ← concrete things: files, data models, routes
  concepts/            ← ideas, algorithms, workflows that span files
```
...
- **Links**: Obsidian wikilinks `[[velocity-estimation]]` (filename, no path,
  no extension — Obsidian resolves it anywhere in the vault). Link liberally;
...
- **Code references**: cite real symbols and line-anchored locations as
  `` `app.py › suggest_crop()` `` so a reader can jump to the source. Do **not**
  paste large code blocks — summarize behavior and show only the essential
  lines.
...
3. Update the affected entity/concept pages (a single change often touches
   several — e.g. a new route touches [[api-routes]] **and** the concept page
   for what it does).
...
3. If the answer is durable (a comparison, a discovered connection, a gotcha),
   **file it back** as a new page or a section — don't let it vanish into chat.
...
working directory name — *"Supermercado Telefrango (Sem GPS)"* — signals the
