...
camera pointing forward — the only orientation that yields usable footage.

## Criterion (per frame, then smoothed over ~1 s)
Using the nearest accelerometer sample, normalized by `|a|`:
- `ay/|a| > 0.95` — gravity lies almost entirely on the phone's **y-axis** →
  portrait (see axis convention in [[clip-data-model]]).
- `az/|a| < 0.12` — little gravity on **z** → screen not facing up/down, i.e.
  camera roughly horizontal/forward.

Both signals are smoothed with a 1-second moving average (`W = fps·1`) before
...
(13.0s, 11.5s, none, 9.0s, none) and reproduced **5/5** — see the docstring in
...
- `has_vertical = vertical.any()` — if a clip never goes vertical, the API flags
