...
per-frame velocity **without GPS** — essential for the indoor domain (see
[[overview]]). It fuses accelerometer + gyroscope into a world-frame velocity by
dead reckoning.

## Pipeline
1. **Initial gravity & orientation.** Average the first ≤200 accel samples to
...
2. **Gyro integration.** For each gyro sample, form the incremental rotation
   quaternion from angle-axis (`ω·dt`) and compose: `q = q ⊗ dq`. Samples with
   `dt ≤ 0` or `dt > 0.05 s` (gaps) are skipped — velocity is carried forward.
...
4. **Linear acceleration.** `a_world = q·a_phone − [0,0,G]` removes gravity.
5. **Deadband + integrate + decay.** Components with `|a| < 0.25` are zeroed
   (noise gate); `vel += a_world·dt`; then `vel *= 0.998` each step.
6. **Sample to frames.** The velocity track is sampled at each frame time →
   `{vx, vy, vz, speed=|v|}`.
...
speed profile**, not a metrically exact velocity — good enough to spot
...
- *Could [[walking-detection]] use this fused `speed` instead of raw accel std?*
  — currently it does not; worth comparing during a [[log|lint]] pass.
